defmodule Example.Repo.Migrations.AddBm25Stats do
  use Ecto.Migration
  ...
  def up do
    ...
    execute """
    -- Create verses statistics table
    ...
    CREATE OR REPLACE FUNCTION index_document(verse_id BIGINT, verse_text TEXT)
    RETURNS BIGINT AS $$
    DECLARE
        doc_length INTEGER;
        term_counts JSONB;
    BEGIN
        ...
        WITH term_counts_cte AS (
            SELECT * FROM tokenize_and_count(verse_text)
        )
        SELECT
        ...
        INSERT INTO verse_stats (verse_id, length, terms)
        VALUES (verse_id, doc_length, term_counts);
    ...
    CREATE OR REPLACE FUNCTION update_document_index(p_verse_id BIGINT, new_verse_text TEXT)
    RETURNS BIGINT AS $$
    DECLARE
        old_terms JSONB;
    ...
    BEGIN
        -- Get the existing terms for this verse
        SELECT terms INTO old_terms
        FROM verse_stats
        WHERE verse_id = p_verse_id;

        -- If verse doesn't exist, fall back to index_document function
        IF old_terms IS NULL THEN
            RETURN index_document(p_verse_id, new_verse_text);
        END IF;

        -- Calculate new term frequencies
        WITH term_counts_cte AS (
            SELECT * FROM tokenize_and_count(new_verse_text)
        )
        SELECT
            json_object_agg(term, count)::jsonb,
            sum(count)
    ...
    -- CREATE OR REPLACE FUNCTION search_verses_bm25(
    ...
    BEGIN
        ...
        term_scores AS (
            ...
            FROM
                verse_stats d
            JOIN
                verses sect ON sect.id = d.verse_id
            JOIN
    ...
    execute """
    CREATE MATERIALIZED VIEW global_stats AS SELECT COUNT(*) as total_docs, AVG(length) as avg_length FROM verse_stats;
    """
    ...
    execute """
    CREATE INDEX idx_verse_stats_terms ON verse_stats USING gin(terms);
    """

    execute """
    CREATE OR REPLACE FUNCTION index_all_verses()
    RETURNS INTEGER AS $$
    DECLARE
    ...
    BEGIN
        ...
        RAISE NOTICE 'Indexed % verses and refreshed global_stats', indexed_count;
    ...
    execute """
    -- Update only verses that have been modified since last index update
    CREATE OR REPLACE FUNCTION bulk_update_modified_verses()
    RETURNS INTEGER AS $$
    DECLARE
        updated_count INTEGER := 0;
    BEGIN
        -- This assumes you have some way to track when verses were last modified
        -- You might need to add a modified_at column to your verses table
        SELECT COUNT(*) INTO updated_count
        FROM (
            SELECT update_document_index(s.id, s.text)
            FROM verses s
            LEFT JOIN verse_stats ss ON ss.verse_id = s.id
        ...
        RAISE NOTICE 'Updated % modified verses and refreshed global_stats', updated_count;
